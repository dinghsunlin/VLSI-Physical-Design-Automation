...
die_x1 = 0
die_y1 = 0
die_x2 = CS_WIDTH * 4 + M3_WIDTH * (4 * 2) + M3_SPACING * (4 * 3 - 1)
die_y2 = CS_HEIGHT * 4 + M4_WIDTH * (4 * 1) + M4_SPACING * (4 * 2 - 1)
...
for i in range(4):
    for j in range(4):
        ...
        x = i * (CS_WIDTH + M3_WIDTH * 2 + M3_SPACING * 3)
        y = M4_WIDTH + M4_SPACING + j * (CS_HEIGHT + M4_WIDTH + M4_SPACING * 2)
...
for i in range(4):
    for j in range(2):
        ...
        layer = 'ME3'
        x1 = cs_array[i][0]._x + CS_WIDTH + j * M3_WIDTH + (j + 1) * M3_SPACING
        x2 = x1 + M3_WIDTH
        y1 = 0
        y2 = die_y2
...
for i in range(4):
    ...
    x1 = 0
    x2 = die_x2
    y1 = cs_array[0][i]._y - M4_WIDTH - M4_SPACING
    y2 = y1 + M4_WIDTH
...
for i in range(2):
    for j in range(2):
        ...
        inst_name = 'Via34_port2ME3_' + str(i * 2 + j + 0 * 4)
        x = ME3_specialnet[i][j]._x1
        Via34_port2ME3[i * 2 + j][0] = Component(lib_name, inst_name, x, y)
        inst_name = 'Via34_port2ME3_' + str(i * 2 + j + 1 * 4)
        x = ME3_specialnet[3-i][j]._x1
